...
Одним из важных этапов тестирования является тестирование ПО на безопасность. 
...
![WATERFALL](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/51.png)
...
![AGILE](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/5.2.png)
...
![SCRUM](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/5.2.1.png)
...
![KANBAN](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/5.2.2.png)
...
6. ПОДГОТОВКА К РУЧНОМУ ТЕСТИРОВАНИЮ
6.1. АНАЛИЗ ТЕХНИЧЕСКОГО ЗАДАНИЯ ДЛЯ ПРОРАБОТКИ СЦЕНАРИЕВ ТЕСТИРОВАНИЯ
...
Для проведения качественного тестирования были разработаны тест-кейсы на основе имеющихся спецификаций, используя метод «Черного ящика» (Приложение №2)


6.3. ВЫПОЛНЕНИЕ РУЧНОГО ТЕСТИРОВАНИЯ С ПРИМЕНЕНИЕМ ТЕХНИК ТЕСТ-ДИЗАЙНА
...
![Линейные классы эквивалентности и граничные значения ](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/731.png)
...
![Нелинейные классы эквивалентности](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/7311.png)
...
С помощью техники попарного тестирования создаем проверки так, чтобы выполнились все возможные комбинации каждой пары входных параметров.
Данная техника не обеспечивает исчерпывающее тестирование, но эффективна для поиска ошибок.
...
![Регистрация нового пользователя](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/регистрация.png)
![Регистрация нового пользователя](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/регистрация%20успешно.png)

Теперь войдем в «Личный кабинет», как зарегистрированный пользователь, протестируем форму «Вход в систему»:

![Вход в систему](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/вход.png)

Вход в систему, зарегистрированного пользователя прошел успешно: 

![Вход в систему](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/добро%20пожаловать.png)
...
Для этого были созданы тест-кейсы проверки форм «Регистрация» и «Вход в систему» (Приложение 2). 
...
Т.к. тестировались несколько параметров одновременно, применили метод Доменного анализа, с помощью которого было сформировано 61 тест-кейс для формы регистрации нового пользователя.
Суть доменного тестирования заключается в том, чтобы разделить набор условий тестирования на те значения, которые можно считать одинаковыми, и за счет этого протестировать эффективней.
...
Затем выявить конкретный набор значений и выбрать из них наиболее показательные, представляющие каждую группу, включая обязательно границы. Здесь мы уже определяем значения, которые будем проверять.
Далее необходимо скомбинировать эти значения таким образом, чтобы отдельные параметры можно было протестировать одновременно. Для этого с помощью инструмента Pairwise было сформировано 61 тест-кейс для проведения тестирования с позитивными проверками. Все полученные данные свели в общую таблицу с помощью Доменного анализа. Таблица PairWise и результирующая таблица Доменного анализа находятся в ПРИЛОЖЕНИИ ___.

После прохождения всех тест-кейсов, были заведены баг-репорты на все  найденные баги (Приложение 7). После исправления разработчиками будет выполнена повторная проверка. Если все будет работать без багов, доработки больше не понадобятся. 
...
2. Форма «Регистрация»: После регистрации, пользователь попадает на страницу, где предлагается изучить Инструкцию для регистрации:

![Вход в систему](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/юб1.png)

Перейдя в личные сообщения, пользователь видит сообщение, где указывается снова о том, что нужно кликнуть пиктограмму:

![Вход в систему](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/юб2.png)

И только после этого пользователь попадает на страницу, где необходимо заполнить персональные данные для дальнейшего использования приложения. 
...
![Редактирование адреса](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/юб3.png)


Новый пользователь впервые вносит свои данные, а не редактирует их.

![Редактирование адреса](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/юб4.png)
...
![Редактирование адреса](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/юб5.png)
...
![ТЕСТИРОВАНИЕ ЛОКАЛИЗАЦИИ](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/лок1.png)

Перевод на английский язык всего сайта произошел неполностью, информация по операторам мобильной связи осталась на русском языке. Тоже самое наблюдаем и при переходе на белорусский язык (БЕЛ): 

![ТЕСТИРОВАНИЕ ЛОКАЛИЗАЦИИ](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/лок2.png)
...
![ТЕСТИРОВАНИЕ ЛОКАЛИЗАЦИИ](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/лок3.png)
...
![ТЕСТИРОВАНИЕ ЛОКАЛИЗАЦИИ](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/лок4.png)


8.5. Тестирование пользовательского интерфейса

При тестировании пользовательского интерфейса были сделаны общие проверки:
...
тестирование прошло частично успешно, однако при переходе с поля дата в последующие окна, окно с выбором даты не пропадает, что визуально мешает дальнейшему переходу по полям.

![Тестирование интерфейса](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/интерфейс0.png)
...
•	Проверить выделение текста в полях с помощью Ctrl+A /  Shift+стрелка
тестирование прошло успешно
...
тестирование прошло успешно только при клике

•	Если элемент был выбран, то должен находиться сверху либо обозначен, что он выбран
тестирование прошло успешно

•	Расположение поп-ап по центру окна
тестирование прошло успешно

![Тестирование интерфейса](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/интерфейс.png)
...
Необходимо протестировать функционал, который на текущий момент реализован на стороне клиента – Форма регистрации пользователя. 
Команда тестировщиков. 
В таблице указан список специалистов, задействованных в тестировании проекта, с указанием закрепленного за каждым участником тестируемого функционала.

![Отчет](https://github.com/NatalyaKregel/Graduate_work_KregelNA/blob/main/отчет.png)
...
Исходя из проведенного тестирования функционала Форма регистрации пользователя, можно сделать вывод, что тестирование формы выявило множество багов, которые могут повлиять на удобство использования пользователями данного продукта. Релизить можно после исправления ошибок (хотя бы major). 
